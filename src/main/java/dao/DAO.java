...
public class DAO extends Conn implements Serializable{
	...
	public List<DataBean>selectData(int page,String status,String status1,String status2,String status3){
		List<DataBean> data = new ArrayList<DataBean>();
		try {
			String sql = "select student_id,kanji_name,furigana from gakusei_master where enrollment_status like ? or enrollment_status like ? or enrollment_status like ? or enrollment_status like ? limit ?, ?";
			PreparedStatement st = con.prepareStatement(sql);
			int baseRow= (page -1) * MAXROW;
	...
	public int insertData(DataBean bean) {
		int result = -1;
		try {
			String sql="insert into gakusei_master(student_id, kanji_name,furigana,birth_day,zip_code,address,telephone,mail,enrollment_status,enrollment_status_day,parents_kanji_name,parents_furigana,parents_zipcode,parents_address,parents_telephone,parents_mail) values(?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			PreparedStatement st = con.prepareStatement(sql);
			st.setInt(1, bean.getId());
			st.setString(2, bean.getName());
			st.setString(3,bean.getFurigana());
			st.setDate(4, bean.getBirth());
			...
			st.setDate(10, bean.getStatus_day());
	...
	public DataBean getOneRec(String id) {
		...
		try {
			...
			data.setBirth(rs.getDate("birth_day"));
			data.setZip(rs.getString("zip_code"));
			data.setAddress(rs.getString("address"));
			data.setTelephone(rs.getString("telephone"));
			data.setMail(rs.getString("mail"));
			data.setStatus(rs.getInt("enrollment_status"));
			data.setStatus_day(rs.getDate("enrollment_status_day"));
	...
	public int updateData(DataBean bean) {
		int result = -1;
		try {
			String sql = " update gakusei_master set kanji_name=?,furigana=?,birth_day=?,zip_code=?,address=?,telephone=?,mail=?,enrollment_status=?,enrollment_status_day=?,parents_kanji_name=?,parents_furigana=?,parents_zipcode=?,parents_address=?,parents_telephone=?,parents_mail=?  where student_id=?";// SQL文
			...
			st.setDate(9, bean.getStatus_day());
